...
class Sprite:
    ...
    def draw(self, canvas):
        ...
        if self.animated:
            image_center[0] += self.age * self.image_size[0]
...
def rock_spawner():
    ...
    if started and len(rock_group) < 12:
        if dist(my_ship.get_position(), rock_pos) <= 1.05 * (ship_info.get_radius() + asteroid_info.get_radius()):
            rock_spawner()
        else:
...
def group_collide(group, other_object):
    # removes the first element from group that collides with other object
...
def group_group_collide(attacking_group, destroyed_group):
    collision_count = 0
    for sprite in list(attacking_group):
        if group_collide(destroyed_group, sprite):
            collision_count += 1
            attacking_group.remove(sprite)
